...
init_temp_dir() {
    TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/linux-forensics.work.XXXXXX")
    chmod 700 "$TEMP_DIR"
    trap cleanup_temp_dir EXIT
}

# Parse /etc/os-release without sourcing (avoids arbitrary code execution as root).
...
validate_and_init_output() {
    ...
    OUTPUT_DIR="$dir"
    OUTPUT_FILE=$(mktemp "${OUTPUT_DIR}/linux-forensics-XXXXXX.txt")
...
validate_severity() {
    case "$SEVERITY" in
        ...
        *)
            echo "ERROR: Invalid severity '${SEVERITY}'. Use: low, normal, high, urgent, critical" >&2
            exit 1
            ;;
    esac
}
...
require_python3() {
    if ! command -v python3 >/dev/null 2>&1; then
        log_error "python3 is required for AWS Support case creation (safe JSON encoding)"
        return 1
    fi
    return 0
}

build_support_case_json() {
    local json_file="$1" subject="$2" severity="$3" description_file="$4"

    python3 - "$json_file" "$subject" "$severity" "$description_file" <<'PY'
import json
import sys

out_path, subject, severity, desc_path = sys.argv[1:5]
...
build_support_attachment_json() {
    local json_file="$1" report_path="$2" file_name="$3"

    python3 - "$json_file" "$report_path" "$file_name" <<'PY'
import base64
import json
import sys

out_path, report_path, file_name = sys.argv[1:4]
with open(report_path, "rb") as handle:
    encoded = base64.b64encode(handle.read()).decode("ascii")
payload = {"attachments": [{"fileName": file_name, "data": encoded}]}
with open(out_path, "w", encoding="utf-8") as handle:
    json.dump(payload, handle)
PY
}
...
detect_os() {
    if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        DISTRO=$(read_os_release_field "ID" "unknown")
        OS_VERSION=$(read_os_release_field "VERSION_ID" "unknown")
        OS_VERSION_MAJOR=$(echo "$OS_VERSION" | cut -d. -f1)
        OS_NAME=$(read_os_release_field "PRETTY_NAME" "$DISTRO")
...
analyze_glances_overview() {
    if command -v glances >/dev/null 2>&1; then
        ...
        if [[ "$MODE" == "deep" ]]; then
            local glances_json
            glances_json=$(mktemp "${TEMP_DIR}/glances_snapshot.XXXXXX.json")
            echo "" | tee -a "$OUTPUT_FILE"
            ...
            rm -f "$glances_json" 2>/dev/null || true
        fi
...
analyze_disk() {
    ...
    if command -v iostat >/dev/null 2>&1; then
        ...
        local iostat_output
        iostat_output=$(mktemp "${TEMP_DIR}/iostat.XXXXXX.txt")
        iostat -x 1 10 2>/dev/null | tail -n +4 > "$iostat_output" 2>/dev/null || true

        if [[ -s "$iostat_output" ]]; then
            echo "I/O Statistics:" | tee -a "$OUTPUT_FILE"
            cat "$iostat_output" | tee -a "$OUTPUT_FILE"

            # Analyze I/O wait
            local avg_await=$(cat "$iostat_output" | grep -v "^$" | grep -v "Device" | awk '{sum+=$10; count++} END {if(count>0) print sum/count; else print 0}')
            echo "" | tee -a "$OUTPUT_FILE"
    ...
    if [[ "$MODE" == "disk" ]] || [[ "$MODE" == "deep" ]]; then
        ...
        if [[ "$IS_CONTAINER" == "true" ]]; then
            ...
            local test_file
            test_file=$(mktemp "${TEMP_DIR}/forensics_disk_test.XXXXXX")
...
analyze_storage_profile() {
    ...
    if [[ "$MODE" == "deep" ]] || [[ "$MODE" == "disk" ]]; then
        ...
        local test_dir
        test_dir=$(mktemp -d "${TEMP_DIR}/storage_baseline.XXXXXX")
        chmod 700 "$test_dir"
...
create_support_case() {
    ...
    log_info "Attaching full unredacted diagnostic report (required for support analysis)"
    ...
    local case_description
    case_description=$(cat <<EOF
AUTOMATED LINUX FORENSICS REPORT
...
EOF
    ...
    local description_file case_json_file
    description_file=$(mktemp "${TEMP_DIR}/support-description.XXXXXX.txt")
    printf '%s' "$case_description" > "$description_file"
    case_json_file=$(mktemp "${TEMP_DIR}/support-case.XXXXXX.json")
    build_support_case_json "$case_json_file" "$case_subject" "$SEVERITY" "$description_file"
    
    # Create the case
    local case_id
    if ! case_id=$(aws support create-case --cli-input-json "file://${case_json_file}" --query caseId --output text 2>&1); then
        log_error "Failed to create support case: ${case_id}"
        ...
        rm -f "$description_file" "$case_json_file"
        return
    fi
    ...
    # Attach full diagnostic file (unredacted)
    log_info "Attaching diagnostic report..."

    local attachment_json_file report_name
    report_name=$(basename "$OUTPUT_FILE")
    attachment_json_file=$(mktemp "${TEMP_DIR}/support-attachment.XXXXXX.json")
    build_support_attachment_json "$attachment_json_file" "$OUTPUT_FILE" "$report_name"

    local attachment_set_id
    if attachment_set_id=$(aws support add-attachments-to-set --cli-input-json "file://${attachment_json_file}" --query attachmentSetId --output text 2>&1); then
        ...
        log_error "Failed to attach diagnostic report: ${attachment_set_id}"
    fi

    rm -f "$description_file" "$case_json_file" "$attachment_json_file"
...
main() {
    ...
    validate_severity
    init_temp_dir
    validate_and_init_output "$OUTPUT_DIR"
